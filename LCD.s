...
BusyHigh
    LDR R1, [R2]
    AND R3,R1,#0x10
    CMP R3, #0   ;Check bit 4 of SR register, wait till it is a 0
    BNE BusyHigh
    LDR R2, =DC
    LDR R1, [R2]
    BIC R1,#0x40
    STR R1, [R2] ;Clear D/C to a zero
    LDR R2, =SSI0_DR_R
    STR R0,[R2] ;Store input command into the DR register
    LDR R2, =SSI0_SR_R
BusyHigh2
    LDR R1,[R2]
    AND R3,R1,#0x10
    CMP R3,#0 ;Check bit 4 of SR register, wait till itis 0
    BNE BusyHigh2
    BX  LR                          ;   return
...
; Output: none
; Assumes: SSI0 and port A have already been initialized and enabled
data EQU 0  ;local binding input variable data 
writedata
...
BusyLow
    LDR R1, [R2]
    AND R3,R1,#0x02
    CMP R3,#0 ;Check if TNF bit is high, if not keep checking
    BEQ BusyLow
    LDR R2, =DC
    LDR R1, [R2]
    ORR R1,R1,#0x40
    STR R1, [R2] ;Set D/C to a high
    LDR R2,=SSI0_DR_R
    STR R0, [R2]  ;Store data into DR register
    BX  LR                          ;   return
